...
static SpecRetType processSpecAtom(std::string & atom, SpecFlagsType & flags, SpecFunType & fun)
{
   	...
   	if (allocstr.size() >= 1 && (allocstr[0] == 'g' || allocstr[0] == 'G'))
    {
		...
		{
			K::Logger::Logg(C_ERROR, FMT("invalid dial string '%s': no valid group found!") % allocstr.c_str());
        	return SPR_FAIL;
		}
	...
	if (!what.matched())
	{
		K::Logger::Logg(C_ERROR, FMT("invalid dial string '%s': this is not a valid expression.") % allocstr.c_str());
		return SPR_FAIL;
	}
	...
	try
	{
		...
		if (what.matched(3)) //matched [bB]
		{
			...
			{
            	K::Logger::Logg(C_ERROR, FMT("invalid dial string '%s': no such board '%d'.") % allocstr.c_str() % board_id);
				return SPR_FAIL;
			}
		...
		else if (what.matched(6)) //matched [sS]
		{
			...
			{
   	        	K::Logger::Logg(C_ERROR, FMT("invalid dial string '%s': there is no board with serial '%04d'.") % allocstr.c_str() % serial_id);
				return SPR_FAIL;
			}
		...
		else if (what.matched(14)) //matched [rR]
		{
			...
			if (what.matched(17))
			{
				...
				if (!reverse)
				{
					...
					{
					    /*
						std::string call_addr = fxs_pad_orig(base_addr, i - branch_id);
                        //TODO: FXS TREATMENT JUST HERE PLEASE >> WHERE I GO !!!

						branch_to_object_map_type::iterator i = Board::KhompPvt::fxs_branch_map.find(call_addr);

						if (i == Board::KhompPvt::fxs_branch_map.end())
						{
   	    			    	K::Logger::Logg(C_WARNING, FMT("invalid value '%s': there is no such branch number.") % call_addr);
							return SPR_FAIL;
						}
			...
			else
			{
                ...
                /*
				branch_to_object_map_type::iterator i = Board::KhompPvt::fxs_branch_map.find(base_addr);

				if (i == Board::KhompPvt::fxs_branch_map.end())
				{
   	    	    	K::Logger::Logg(C_WARNING, FMT("invalid value '%s': there is no such branch number.") % base_addr);
					return SPR_FAIL;
				}
		...
		{
           	K::Logger::Logg(C_ERROR, FMT("invalid dial string '%s': unknown allocation method.") % allocstr.c_str());
			return SPR_FAIL;
		}
		...
		if (what.matched(9)) // matched something about links/channels [cClL]n|[cC]n-m
		{
			...
			if (what.matched(12))
			{
				...
				{
					K::Logger::Logg(C_ERROR, FMT("invalid dial string '%s': range just allowed for channels.") % allocstr.c_str());
					return SPR_FAIL;
				}
				...
				switch ((what.submatch(10))[0])
				{
					...
					case 'L':
						...
						switch (Globals::k3lapi.device_type(board_id))
						{
							...
							default:
								K::Logger::Logg(C_ERROR, FMT("invalid dial string '%s': board '%d' does not have links.")
									% allocstr.c_str() % board_id);
								return SPR_FAIL;
						}

					default:
						K::Logger::Logg(C_ERROR, FMT("invalid dial string '%s': invalid object specification.") % allocstr.c_str());
						return SPR_FAIL;
				}
			}
		}
	...
	{
		K::Logger::Logg(C_ERROR, FMT("invalid dial string '%s': invalid numeric value specified.") % allocstr.c_str());
		return SPR_FAIL;
	}
...
static SpecRetType processSpecAtoms(std::string & gotatoms, SpecFlagsType & flags, SpecFunType & fun)
{
    ...
    {
        K::Logger::Logg(C_ERROR, FMT("invalid dial string '%s': no allocation string found!") % atoms);
        return SPR_FAIL;
    }
...
Board::KhompPvt * process_dial_string (const char *dial_charv, int *cause)
{
    //TODO: NEED KHOMP LOG, INSTEAD SWITCH_LOG
	...
	{
		K::Logger::Logg(C_ERROR, FMT("invalid dial string '%s': wrong number of separators ('/').") % dial_string);
		return NULL;
	}
	...
	{
		if (dial_string_ok)
            K::Logger::Logg(C_WARNING, "unable to allocate channel -- no free channel found!");
		return NULL;
    }
    ...
    /* reset options/values
        TODO: reset params, who !?
	*/

    //TODO: Parameters for fxs
	//unsigned int opt_size = (pvt->is_fxs() ? 2 : 3);
	//unsigned int opt_arg  = (pvt->is_fxs() ? 1 : 2);

	unsigned int opt_size = 3;
	unsigned int opt_arg  = 2;
	...
	if (dial_args.size() == opt_size)
	{
		     ...
		     opt_arg != options_args.end(); opt_arg++)
		{
			...
			switch (option_item.size())
			{
				...
				default:
	            {
					K::Logger::Logg(C_ERROR, FMT("invalid option specification '%s'.") % str);
	                continue;
	            }
			}
            K::Logger::Logg(C_ERROR, FMT("unknown option name '%s', ignoring...") % str);
        }
	}

    //if (!pvt->is_fxs()) //TODO: checagem de fxs
	//{
		if (dial_args.size() <= 1)
		{
            K::Logger::Logg(C_ERROR, FMT("invalid dial string '%s': missing destination number!") % dial_string);
			return NULL;
		}
