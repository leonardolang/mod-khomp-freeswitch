...
#define OBJ_FMT(dev,obj,msg) \
    FMT("%s (dev=%02d,channel=%03d) " msg) % __SWITCH_FUNC__ % dev % obj

#define OBJ_MSG(dev,obj,msg) \
    "%s (dev=%02d,channel=%03d) " __SWITCH_FUNC__, msg, dev, obj

#define PVT_FMT(tgt,msg) \
    FMT("%s (dev=%02hu,channel=%03hu) " msg) % __SWITCH_FUNC__ % tgt.device % tgt.object

#define PVT_MSG(tgt,msg) \
    "%s (dev=%02hu,channel=%03hu) " __SWITCH_FUNC__,msg, tgt.device, tgt.object
...
#define CASE_DIGITAL_GRP    \
    CASE_R2_SIG:            \
    CASE_RDSI_SIG:          \
    case ksigSIP:           \
    case ksigGSM

#define CASE_TRUNK_GRP      \
    CASE_R2_SIG:            \
    CASE_RDSI_SIG:          \
    case ksigSIP

#define CASE_BRANCH_GRP         \
    case ksigAnalog:            \
    case ksigAnalogTerminal:    \
    case ksigGSM

#define CASE_ANALOG_GRP         \
    case ksigAnalog:            \
    case ksigAnalogTerminal
