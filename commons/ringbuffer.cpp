...
//        fprintf(stderr, "%p> full write: %d/%d [%d/%d]\n", this, amount, _size, reader, writer);
        ...
        writer = writer + amount;
    }

    _writer         = writer;
    _writer_partial = writer * _block;

//    fprintf(stderr, "%p> write end: %d [%d]\n", this, _writer, _writer_partial);

    return true;
}

/* returns the number of itens or bytes (depending on policy) that have been read */
...
{
	/* avoid using different values */
    unsigned int reader = _reader_partial;
	unsigned int writer = _writer_partial;

	unsigned int size = _size * _block;
    unsigned int dest = writer - 1;
...
//        fprintf(stderr, "%p> p partial write: (%d/%d) %d/%d [%d/%d]\n", this, wr1, wr2, amount, size, reader, writer);
        ...
        _writer_partial = ((dest + amount) % size) + 1;
    }
    else
    {
        if (!reader_less && ((reader - writer) <= amount))
...
//        fprintf(stderr, "%p> p full write: %d/%d [%d/%d]\n", this, amount, size, reader, writer);

        /* we are talking about buffers here, man! */
        memcpy((void *) &(buffer[dest]), (const void *) value, amount);

        _writer_partial = writer + amount;
    }

	/* update "full length position" */
	_writer = (unsigned int)floor((double)_writer_partial / (double)_block)+1;

//    fprintf(stderr, "%p> p write end: %d [%d]\n", this, _writer, _writer_partial);
...
//        fprintf(stderr, "%p> p partial read: (%d/%d) %d/%d [%d/%d]\n", this, rd1, rd2, total, size, reader, writer);           
        ...
        /* jump the reader forward */
        _reader_partial = (reader + total) % size;
...
//        fprintf(stderr, "%p> p full read: %d/%d [%d/%d]\n", this, total, size, reader, writer);

        /* we are talking about buffers here, man! */
        memcpy((void *) value, (const void *) &(buffer[_reader]), total);

        /* jump the reader forward */
        _reader_partial = (reader + total);
    }

	_reader = (unsigned int)floor((double)_reader_partial / (double)_block);

//    fprintf(stderr, "%p> p read end: %d [%d]\n", this, _reader, _reader_partial);
